...
function MemojiAvatar({ type, className = "" }: { type: 'landing' | 'chat'; className?: string }) {
  ...
  const [imgSrc, setImgSrc] = useState(() => type === 'landing' ? '/assets/memoji/landing.png' : '/assets/memoji/chat.png');
...
function ProfileImage({ className = "" }: { className?: string }) {
  ...
  if (hasError) {
    return (
      <img
        src="https://ui-avatars.com/api/?name=Shubham+Patil&size=256&background=f1f5f9&color=334155"
        alt="Shubham"
        className={className}
      />
    );
...
export default function App() {
  ...
  const handleQuery = async (query: string, typeHint: ChatMessage['type'] = 'general') => {
      ...
      <AnimatePresence>
        {viewState === 'landing' && (
          ...
          >
            ...
            <div className={`absolute bottom-16 sm:bottom-20 left-1/2 transform -translate-x-1/2 text-[7vw] sm:text-[5.5vw] md:text-[4.5vw] font-bold tracking-tighter whitespace-nowrap select-none transition-colors duration-500 ${theme === 'dark' ? 'text-zinc-800/30' : 'text-slate-200/40'
              }`}>
        ...
        <div className="flex-1">
          <AnimatePresence>
            {viewState === 'landing' && (
              <motion.a
                initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                href="https://drive.google.com/file/d/1MjR5sYQge4aYzz7yamTjgoVSYj9Q2W2w/view?usp=sharing"
                target="_blank"
...
function LandingView({ onQuery, theme }: { onQuery: (q: string, t: ChatMessage['type']) => void, theme?: string }) {
  ...
  const dynamicPlaceholder = useTypewriterPlaceholder([
    ...
    "Tell me about your experience at Felix-IT Systems...",
    ...
    <>
      ...
      >
        <div className="text-center mb-6">
          <h2 className={`text-2xl sm:text-3xl font-serif italic mb-3 transition-colors duration-500 ${theme === 'dark' ? 'text-zinc-300' : 'text-slate-700'}`}>Hey, I'm Shubham 👋</h2>
          <h1 className={`text-5xl sm:text-7xl md:text-8xl font-serif font-bold tracking-tight transition-colors duration-500 ${theme === 'dark' ? 'text-zinc-100' : 'text-slate-900'}`}>Web Developer</h1>
        </div>

        <motion.div className="w-24 h-24 mb-8 flex items-center justify-center text-5xl">
          <MemojiAvatar type="landing" />
        </motion.div>
...
function ChatView({
  ...
  onQuery: (query: string, typeHint?: ChatMessage['type']) => Promise<void>,
  isTyping: boolean,
      ...
      >
        ...
        <div className="w-full relative px-2">
          <input
            ...
            onKeyDown={(e) => e.key === 'Enter' && handleSubmit()}
            placeholder={dynamicPlaceholder}
            className={`w-full shadow-[0_8px_32px_rgba(0,0,0,0.06)] border rounded-full py-4 pl-6 pr-14 text-[15px] focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500/50 transition-all placeholder-zinc-400 ${theme === 'dark'
...
function MagnifyingItem({ children, mouseX }: { children: React.ReactNode, mouseX?: MotionValue<number> }) {
  ...
  return (
    <motion.div
      ref={ref}
      style={{ scale, marginLeft: margin, marginRight: margin, zIndex: zIndex as unknown as number }}
          ...
          >
            ...
            <div className={`space-y-6 text-lg leading-relaxed transition-colors duration-500 ${theme === 'dark' ? 'text-zinc-350' : 'text-slate-600'}`}>
              <div>
                ...
                <p>
                  Welcome to my <strong>interactive AI Portfolio.</strong><br />
                  I'm excited to share my journey as a Full-Stack Developer. Ask me about my MERN stack projects, AI integrations, internship experience, technical skills, or career goals—I'm here to help!
                </p>
              </div>
              <div>
                <h3 className={`text-xl font-bold mb-2 transition-colors duration-500 ${theme === 'dark' ? 'text-zinc-100' : 'text-slate-900'}`}>Why?</h3>
                <p>
                  A static portfolio only shows information.<br />
                  This AI portfolio creates an interactive experience tailored to your interests.<br />
                  <strong>Ask about my projects, technical skills, internship, or career journey, and get answers instantly.</strong>
                </p>
              </div>
            </div>
  ...
  return (
    <div className="w-full flex flex-col md:flex-row items-center md:items-start gap-8 mb-8">
      ...
      <div className="flex-1 flex flex-col text-left">
        <h1 className={`text-4xl font-bold mb-2 transition-colors duration-500 ${theme === 'dark' ? 'text-zinc-100' : 'text-slate-900'}`}>Shubham Patil</h1>
        <p className={`mb-4 font-medium transition-colors duration-500 ${theme === 'dark' ? 'text-zinc-400' : 'text-slate-505'}`}>Web Developer <span className="mx-2">•</span> India</p>

        <p className={`leading-relaxed mb-6 transition-colors duration-500 ${theme === 'dark' ? 'text-zinc-300' : 'text-slate-700'}`}>
          Hey 👋<br />
          I'm <span>Shubham Patil</span>, a Full-Stack Web Developer who loves building fast, scalable, and responsive web applications. I specialize in the MERN stack, REST APIs, and AI-powered web applications, with a passion for creating clean, efficient, and user-friendly digital experiences.
        </p>

        <div className="flex flex-wrap gap-2">
          <span className="bg-blue-600 text-white px-4 py-1.5 rounded-full text-[13px] font-normal shadow-sm">Web Developer</span>
          <span className="bg-blue-600 text-white px-4 py-1.5 rounded-full text-[13px] font-normal shadow-sm">Rest API</span>
          <span className="bg-blue-600 text-white px-4 py-1.5 rounded-full text-[13px] font-normal shadow-sm">React</span>
          <span className="bg-blue-600 text-white px-4 py-1.5 rounded-full text-[13px] font-normal shadow-sm">Node/Express</span>
          <span className="bg-blue-600 text-white px-4 py-1.5 rounded-full text-[13px] font-normal shadow-sm">MongoDB/MySQL</span>
        </div>
      </div>
    </div>
  );
...
function ContactCard({ theme }: { theme?: string }) {
  const socials = [
    { name: 'LinkedIn', url: 'https://www.linkedin.com/in/shubham-patil-9773032b4/' },
    { name: 'Github', url: 'https://github.com/04Shubhampatil' },
    { name: 'Discord', url: 'https://discord.com/users/shubhampatil_04_91099' },
    { name: 'X', url: 'https://x.com' },
    { name: 'Instagram', url: 'https://www.instagram.com/the.shu_bham_patil/#' }
  ];
